"""얼굴 검출(YuNet) + 모자이크 마스킹. 원본은 메모리에서만 처리한다."""
...
MODEL_PATH = str(_DIR / "face_detection_yunet_2023mar.onnx")
MODEL_URL = (
...
def ensure_model(path: str = MODEL_PATH, url: str = MODEL_URL) -> str:
    if os.path.exists(path):
        return path
    # stdout은 CLI에서 JPEG 바이너리로 쓰이므로 로그는 stderr만 사용
    print(f"모델 다운로드 중: {url}", file=sys.stderr)
